...
model = Sequential()  
model.add(LSTM(40, input_dim=3, input_length=800, return_sequences=True))
model.add(Flatten())
...
# print(model.summary())
# plot_model(model, to_file='flat_model.png')
...
checkpoint = ModelCheckpoint("flat_model.hdf5", monitor='val_loss', verbose=1, save_best_only=True, mode='min')
...
plt.savefig('flat_matchscore.png')
plt.close()
...
plot_confusion_matrix(cnf_matrix, classes=[x for x in range(len(CLASS_NAMES))], fileName='flat_confMat.png')
